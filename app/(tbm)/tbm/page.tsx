              ...
              <Link href={`/tbm/${r.id}`}>
                <div className="bg-white rounded-2xl p-4 shadow-sm active:opacity-80 pr-14">
                  ...
                  <div className="flex items-center gap-1.5 mb-0.5">
                    {normalizeType(r.taskType) === "용역" && <span className="text-[10px] px-1.5 py-0.5 rounded font-bold bg-green-100 text-green-700 shrink-0">[용역]</span>}
                    {normalizeType(r.taskType) === "자체진단" && <span className="text-[10px] px-1.5 py-0.5 rounded font-bold bg-blue-100 text-blue-700 shrink-0">[자체진단]</span>}
                    <p className="text-sm font-bold text-gray-900 truncate">{r.projectName || r.contractorName || "미입력"}</p>
                  </div>
