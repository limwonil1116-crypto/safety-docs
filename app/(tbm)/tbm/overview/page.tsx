...
function parsePhotos(photoUrl: string): PhotoItem[] {
  ...
  try {
    ...
    return Array.isArray(p) ? p : [{ url: photoUrl, caption: "" }];
  } catch {
  ...
  const navers = [
    {
      ...
      onClick: () => {
        window.location.href = lat && lng ? `kakaomap://look?p=${lat},${lng}` : `kakaomap://search?q=${encoded}`;
    ...
    },
    {
      ...
      onClick: () => {
        window.location.href = lat && lng ? `tmap://route?goalname=${encoded}&goaly=${lat}&goalx=${lng}` : `tmap://search?name=${encoded}`;
    ...
    },
    {
      ...
      onClick: () => {
        window.location.href = lat && lng ? `nmap://route/car?dlat=${lat}&dlng=${lng}&dname=${encoded}&appname=safety-docs` : `nmap://search?query=${encoded}&appname=safety-docs`;
...
export default function TbmOverviewPage() {
  ...
  const handleSelectRegion = (region: RegionStat) => {
    ...
    setSelectedRegion(region); setSelectedReport(null);
    ...
    mapObjRef.current.setCenter(new window.kakao.maps.LatLng(centerLat, centerLng));
    ...
    setTimeout(() => {
      region.reports.forEach((r,i) => {
        ...
        const label=typeTag+(r.projectName||r.facilityName||r.contractorName||`${i+1}번`);
        const div=document.createElement("div");
        ...
        const overlay=new window.kakao.maps.CustomOverlay({position:new window.kakao.maps.LatLng(lat,lng),content:div,map:mapObjRef.current,zIndex:5});
  ...
  return (
    <div className="min-h-screen" style={{ background: "#f0f4f8" }}>
      {naviTarget && <NaviModal address={naviTarget.address} lat={naviTarget.lat} lng={naviTarget.lng} onClose={() => setNaviTarget(null)} />}

      <div className="bg-white border-b border-gray-200 px-4 py-3">
        <div className="flex items-center justify-between mb-2">
          <h1 className="text-base font-bold text-gray-900">TBM 현황 관제</h1>
          {selectedRegion && <button onClick={handleBackToAll} className="text-xs text-blue-500 flex items-center gap-1"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15 18 9 12 15 6"/></svg>전체보기</button>}
        </div>
        <div className="flex items-center justify-center gap-3 mb-2">
          <button onClick={() => moveDate(-1)} disabled={loading} className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center text-gray-600 disabled:opacity-40"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15 18 9 12 15 6"/></svg></button>
          <input type="date" value={date} onChange={e => setDate(e.target.value)} className="text-sm font-bold text-gray-900 border border-gray-200 rounded-lg px-3 py-1.5 focus:outline-none" />
          <button onClick={() => moveDate(1)} disabled={loading} className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center text-gray-600 disabled:opacity-40"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg></button>
        </div>
              ...
              <table className="text-xs" style={{ minWidth: "900px", width: "100%" }}>
                <thead>
                  <tr style={{background:"#f1f5f9"}}>
                    <th className="text-center px-2 py-2.5 text-gray-500 font-semibold border-b border-gray-200 whitespace-nowrap" style={{width:"36px"}}>NO</th>
                    <th className="text-left px-3 py-2.5 text-gray-600 font-semibold border-b border-gray-200 whitespace-nowrap" style={{width:"160px"}}>사업명</th>
                    <th className="text-left px-3 py-2.5 text-gray-600 font-semibold border-b border-gray-200 whitespace-nowrap" style={{width:"60px"}}>사진</th>
                    <th className="text-left px-3 py-2.5 text-gray-600 font-semibold border-b border-gray-200 whitespace-nowrap" style={{width:"160px"}}>교육내용</th>
                    <th className="text-left px-3 py-2.5 text-gray-600 font-semibold border-b border-gray-200 whitespace-nowrap" style={{width:"150px"}}>위험요인</th>
                    <th className="text-left px-3 py-2.5 text-gray-600 font-semibold border-b border-gray-200 whitespace-nowrap" style={{width:"90px"}}>회사명</th>
                    <th className="text-left px-3 py-2.5 text-gray-600 font-semibold border-b border-gray-200 whitespace-nowrap" style={{width:"100px"}}>주소</th>
                    <th className="text-center px-2 py-2.5 text-gray-600 font-semibold border-b border-gray-200 whitespace-nowrap" style={{width:"55px"}}>투입인원</th>
                    <th className="text-left px-3 py-2.5 text-gray-600 font-semibold border-b border-gray-200 whitespace-nowrap" style={{width:"70px"}}>투입장비</th>
                    <th className="text-left px-3 py-2.5 text-gray-600 font-semibold border-b border-gray-200 whitespace-nowrap" style={{width:"80px"}}>위험공종</th>
                    <th className="text-center px-2 py-2.5 text-gray-600 font-semibold border-b border-gray-200 whitespace-nowrap" style={{width:"80px"}}>소장</th>
                  </tr>
                </thead>
                <tbody>
                  {regionDisplayReports.length === 0 ? (
                    ...
                    const photos = parsePhotos(r.photoUrl);
                    return (
                      <tr key={r.id} onClick={() => setSelectedReport(r)}
                        ...
                        <td className="px-3 py-3 whitespace-nowrap">
                          ...
                          {r.band && <span className="text-[10px] text-gray-400 ml-1">{r.band}</span>}
                        </td>
                        <td className="px-3 py-3">
                          {photos.length > 0 ? (
                            <div className="relative">
                              <img src={photos[0].url} alt="사진" className="w-14 h-10 object-cover rounded-lg border border-gray-200" />
                              {photos.length > 1 && <span className="absolute bottom-0.5 right-0.5 bg-black/60 text-white text-[8px] px-1 rounded">{photos.length}장</span>}
                            </div>
                          ) : (
                        ...
                        <td className="px-3 py-3 text-gray-700">
                          <p className="line-clamp-2 text-[11px] leading-snug whitespace-nowrap overflow-hidden text-ellipsis max-w-[150px]">{r.workToday||"-"}</p>
                        </td>
                        <td className="px-3 py-3 text-gray-600">
                          {(r.riskFactor1||r.mainRiskFactor) ? (
                            <div className="text-[10px] leading-snug space-y-0.5">
                              {r.riskFactor1 && <p className="whitespace-nowrap overflow-hidden text-ellipsis max-w-[140px]">• {r.riskFactor1}</p>}
                              {r.riskFactor2 && <p className="whitespace-nowrap overflow-hidden text-ellipsis max-w-[140px]">• {r.riskFactor2}</p>}
                              {r.mainRiskFactor && <p className="text-amber-600 font-medium whitespace-nowrap overflow-hidden text-ellipsis max-w-[140px]">★ {r.mainRiskFactor}</p>}
                            </div>
                          ) : <span className="text-gray-300">-</span>}
                        </td>
                        <td className="px-3 py-3 text-gray-700 text-[11px] whitespace-nowrap">{r.contractorName||"-"}</td>
                        <td className="px-3 py-3">
                          {r.workAddress ? (
                            <button onClick={e => { e.stopPropagation(); setNaviTarget({address:r.workAddress,lat:r.workLatitude,lng:r.workLongitude}); }}
                              className="text-blue-500 text-left flex items-start gap-0.5 hover:text-blue-700">
                              <svg className="mt-0.5 shrink-0" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                              <span className="text-[10px] whitespace-nowrap overflow-hidden text-ellipsis max-w-[85px] underline">{r.workAddress}</span>
                            </button>
                        ...
                        <td className="px-3 py-3 text-gray-500 text-[10px] whitespace-nowrap overflow-hidden text-ellipsis max-w-[65px]">{r.equipment||"-"}</td>
                        <td className="px-3 py-3">
                          {isHigh ? <span className="inline-block px-1.5 py-0.5 rounded bg-red-100 text-red-600 text-[10px] font-bold whitespace-nowrap">{r.riskType}</span>
                            : <span className="text-gray-400 text-[10px] whitespace-nowrap">{r.riskType==="해당없음"?"-":r.riskType||"-"}</span>}
                        </td>
                        <td className="px-2 py-3 text-center text-[11px] whitespace-nowrap">
      ...
      {selectedReport && (
        <div className="p-4 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => window.open(`/api/tbm/${selectedReport.id}/pdf`, "_blank")}
                className="text-xs px-3 py-1.5 rounded-lg border border-red-200 text-red-600 flex items-center gap-1 hover:bg-red-50">
          ...
          {/* 멀티 사진 */}
          {parsePhotos(selectedReport.photoUrl).length > 0 && (
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              <h3 className="text-xs font-bold text-gray-500 mb-2">
                TBM 현장사진 <span className="text-gray-400 font-normal">{parsePhotos(selectedReport.photoUrl).length}장</span>
              </h3>
              <div className="space-y-2">
                {parsePhotos(selectedReport.photoUrl).map((photo, idx) => (
