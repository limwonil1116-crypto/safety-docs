  ...
  const Field = ({ label, value }: { label: string; value?: string | number | boolean | null }) => {
    ...
    return (
      <div className="flex gap-3 py-1.5 border-b border-gray-50">
        ...
        <span className="text-sm text-gray-900 font-medium">{typeof value === "boolean" ? (value ? "사용중" : "사용안함") : String(value)}</span>
      </div>
    );
  };
    ...
    <div className="min-h-screen" style={{ background: "#f0f4f8" }}>
      ...
      <div className="p-4 space-y-4">
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          <Field label="교육 시간" value={report.eduStartTime && report.eduEndTime ? `${report.eduStartTime} ~ ${report.eduEndTime}` : null} />
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          <h3 className="text-sm font-bold text-gray-900 mb-3">🏗️ 작업정보</h3>
        ...
        {(report.riskFactor1 || report.riskFactor2 || report.riskFactor3 || report.mainRiskFactor) && (
          <div className="bg-white rounded-2xl p-4 shadow-sm">
            <h3 className="text-sm font-bold text-gray-900 mb-3">⚠️ 위험요인 및 대책</h3>
            ...
            {(report.riskElement1 || report.riskElement2 || report.riskElement3) && (
              <div className="mt-2">
                ...
                {[1,2,3].map(n => report[`riskElement${n}`] && <p key={n} className="text-sm text-gray-700">• {report[`riskElement${n}`]}</p>)}
              </div>
            )}
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          <h3 className="text-sm font-bold text-gray-900 mb-3">✍️ 교육담당자</h3>
