  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const factorLabels: Record<string,string> = {
        factorNarrowAccess:"접근통로 협소", factorSlippery:"미끄러운 지반", factorSteepSlope:"급경사면",
        factorWaterHazard:"익수·유수", factorRockfall:"낙석", factorNoRailing:"안전난간 미설치",
        factorLadderNoGuard:"사다리 방호울 미설치", factorSuffocation:"질식·산소결핍",
        factorElectricFire:"감전·전기화재", factorSparkFire:"불꽃·화재",
      };
      const checkedFactors = Object.entries(factorLabels).filter(([k]) => !!(form as any)[k]).map(([,v]) => v);
      const prompt = [
        "당신은 한국농어촌공사 안전관리 전문가입니다.",
        "아래 내용을 분석해 위험요소·개선대책·재해형태를 3개 항목으로 작성하세요.",
        "작업내용: " + (form.workContent || "미입력"),
        "작업장소: " + (form.workLocation || "미입력"),
        "위험공종: " + (riskItems.join(", ") || "없음"),
        "예상위험요소: " + (checkedFactors.join(", ") || "없음"),
        "",
        "정확히 아래 JSON만 출력하세요:",
        '[{"riskFactor":"위험요소1","improvement":"개선대책1","disasterType":"재해형태1"},{"riskFactor":"위험요소2","improvement":"개선대책2","disasterType":"재해형태2"},{"riskFactor":"위험요소3","improvement":"개선대책3","disasterType":"재해형태3"}]',
        "재해형태: 낙상/추락/협착/감전/화재/익수/질식 중 선택",
      ].join("\n");
    ...
    } catch (e: any) { alert("AI 생성 오류: " + (e.message||"오류")); }
    finally { setLoading(false); }
  };
  return (
    <button onClick={handleGenerate} disabled={loading}
      ...
      {loading
        ? <><svg className="animate-spin" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>AI 생성 중...</>
        : <>✨ AI 위험요소·개선대책·재해형태 자동생성</>}
    </button>
  );
}
