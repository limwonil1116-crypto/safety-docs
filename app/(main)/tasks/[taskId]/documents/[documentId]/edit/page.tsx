  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm">
        ...
        <div className="space-y-3">
          {/* ✅ 2번: 순서 + placeholder 샘플 */}
          <FormInput label="작업대상 시설물" required>
            <input type="text" value={form.facilityName} onChange={e => onChange("facilityName", e.target.value)}
              placeholder="예) 예당저수지 복통 양수장" className={inputClass + " mb-1.5"} />
            <input type="text" value={form.facilityLocation} onChange={e => onChange("facilityLocation", e.target.value)}
              placeholder="지도에서 선택하거나 직접 입력" className={inputClass + " mb-1.5"} />
            <LocationField workLatitude={workLatitude} workAddress={workAddress} onOpenLocation={onOpenLocation} onClearLocation={onClearLocation} />
          </FormInput>
          <FormInput label="작업위치">
            <input type="text" value={form.workPosition} onChange={e => onChange("workPosition", e.target.value)}
              placeholder="예) 여수로" className={inputClass} />
          </FormInput>
          <FormInput label="작업공종">
            <textarea value={form.workContents} onChange={e => onChange("workContents", e.target.value)}
              rows={2} placeholder="예) 방수로 옹벽 재료조사" className={textareaClass} />
          </FormInput>
          <FormInput label="위험요소">
            <textarea value={form.riskFactors} onChange={e => onChange("riskFactors", e.target.value)}
              rows={2} placeholder="예) 그물부 조사시 미끄러짐" className={textareaClass} />
          </FormInput>
          <FormInput label="개선대책">
            <textarea value={form.improvementMeasures} onChange={e => onChange("improvementMeasures", e.target.value)}
              rows={2} placeholder="예) 안전난간에 안전로프 설치" className={textareaClass} />
          </FormInput>
          {/* ✅ 3번: 관리자 직급 삭제 */}
          <FormInput label="시설 관리자">
            <input type="text" value={form.facilityManager} onChange={e => onChange("facilityManager", e.target.value)}
              placeholder="예) 00지사 0급 홍길동" className={inputClass} />
          </FormInput>
        </div>
      </div>
      {/* ✅ 4번: 참여자 역할 4개 */}
            ...
            <div key={idx} className="border border-gray-200 rounded-xl p-3">
              <div className="flex items-center justify-between mb-2">
                <select value={p.role} onChange={e => updateP(idx, "role", e.target.value)}
                  className="text-xs px-2 py-1 border border-gray-200 rounded-lg text-gray-700 bg-white focus:outline-none">
                ...
                {idx >= 1 && (
                  <button onClick={() => onChange("participants", form.participants.filter((_, i) => i !== idx))}
              ...
              <div className="grid grid-cols-2 gap-2">
                <FormInput label="성명"><input type="text" value={p.name} onChange={e => updateP(idx, "name", e.target.value)}
                  placeholder="홍길동" className={inputClass} /></FormInput>
                <FormInput label="연락처"><input type="tel" value={p.phone} onChange={e => updateP(idx, "phone", e.target.value)}
                  placeholder="010-0000-0000" className={inputClass} /></FormInput>
              </div>
              {/* 기타 선택 시 직접 입력 */}
              {p.role === "기타" && (
                <input type="text" placeholder="역할 직접 입력" className={"mt-2 " + inputClass}
                  onChange={e => updateP(idx, "role", e.target.value)} />
              )}
            </div>
          ))}
        </div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-3">
          <FormInput label="소속">
            <input type="text" value={form.applicantOrg} onChange={e => onChange("applicantOrg", e.target.value)}
              placeholder="한국농어촌공사 내포지사" className={inputClass} />
          </FormInput>
          <FormInput label="신청자 성명" required>
            <input type="text" value={form.applicantName} onChange={e => onChange("applicantName", e.target.value)}
...
export default function DocumentEditPage() {
  ...
  const [taskInfo, setTaskInfo] = useState<{ startDate?: string; endDate?: string } | null>(null);
