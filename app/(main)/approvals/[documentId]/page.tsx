...
function GasRowInput({ row, idx, onChange }: { row: any; idx: number; onChange: (idx: number, field: string, value: string) => void }) {
  const NumSpinner = ({ field }: { field: string }) => {
    const val = parseInt(row[field] || "0", 10) || 0;
    return (
      <div className="flex flex-col items-center">
        <button type="button" onClick={() => onChange(idx, field, String(val + 1))} className="w-8 h-7 flex items-center justify-center rounded-t-lg border border-gray-200 bg-gray-50 hover:bg-gray-100 text-gray-500 text-xs">▲</button>
        <input type="number" value={row[field] || ""} onChange={e => onChange(idx, field, e.target.value)} className="w-12 h-8 text-center text-sm border-x border-gray-200 bg-white focus:outline-none focus:ring-1 focus:ring-blue-400 [-moz-appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none" />
        <button type="button" onClick={() => onChange(idx, field, String(Math.max(0, val - 1)))} className="w-8 h-7 flex items-center justify-center rounded-b-lg border border-gray-200 bg-gray-50 hover:bg-gray-100 text-gray-500 text-xs">▼</button>
      </div>
    );
  };
  const GasInput = ({ field }: { field: string }) => (
    <input type="text" value={row[field] || ""} onChange={e => onChange(idx, field, e.target.value)} className="w-full px-2 py-2 text-sm text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white" />
  );
  return (
    <div className="bg-gray-50 rounded-xl p-3 space-y-3 border border-gray-100">
      <div className="flex items-center gap-3">
        <span className="text-xs font-semibold text-gray-700 w-6 shrink-0">{row.time}</span>
        <div className="flex gap-4">
          <div className="flex flex-col items-center gap-1"><NumSpinner field="hour" /><span className="text-xs text-gray-500">시</span></div>
          <div className="flex flex-col items-center gap-1"><NumSpinner field="minute" /><span className="text-xs text-gray-500">분</span></div>
        </div>
      </div>
      <div>
        <p className="text-xs font-medium text-gray-600 mb-2">측정 농도</p>
        <div className="grid grid-cols-2 gap-2">
          <div className="flex flex-col gap-1"><label className="text-[10px] text-gray-500">산소 O₂ (%)</label><GasInput field="o2" /></div>
          <div className="flex flex-col gap-1"><label className="text-[10px] text-gray-500">이산화탄소 CO₂ (%)</label><GasInput field="co2" /></div>
          <div className="flex flex-col gap-1"><label className="text-[10px] text-gray-500">황화수소 H₂S (ppm)</label><GasInput field="h2s" /></div>
          <div className="flex flex-col gap-1"><label className="text-[10px] text-gray-500">일산화탄소 CO (ppm)</label><GasInput field="co" /></div>
          <div className="flex flex-col gap-1"><label className="text-[10px] text-gray-500">폭발하한 EX (%)</label><GasInput field="ex" /></div>
        </div>
      </div>
      <div className="grid grid-cols-3 gap-2">
        <div><label className="text-xs text-gray-500 mb-1 block">측정자</label><input type="text" value={row.measurer || ""} onChange={e => onChange(idx, "measurer", e.target.value)} className="w-full px-2 py-2 text-xs text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white" /></div>
        <div><label className="text-xs text-gray-500 mb-1 block">입장(명)</label><input type="number" min="0" value={row.entryCount || ""} onChange={e => onChange(idx, "entryCount", e.target.value)} className="w-full px-2 py-2 text-xs text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white" /></div>
        <div><label className="text-xs text-gray-500 mb-1 block">퇴장(명)</label><input type="number" min="0" value={row.exitCount || ""} onChange={e => onChange(idx, "exitCount", e.target.value)} className="w-full px-2 py-2 text-xs text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white" /></div>
      </div>
    </div>
  );
}
