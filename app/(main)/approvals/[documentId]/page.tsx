    ...
    <div className="bg-white rounded-2xl p-4 shadow-sm">
      ...
      {canAdd && (
        <div className="flex gap-2 mt-2 pt-3 border-t border-gray-100">
          <button onClick={() => cameraRef.current?.click()} disabled={uploading} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-gray-300 text-sm text-gray-600 hover:bg-gray-50 disabled:opacity-50">移대찓??/button>
          <button onClick={() => galleryRef.current?.click()} disabled={uploading} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-gray-300 text-sm text-gray-600 hover:bg-gray-50 disabled:opacity-50">媛ㅻ윭由?/button>
        </div>
      )}
...
function ConfinedNextModal({ documentId, action, onClose, onAssigned }: { documentId: string; action: "PLAN_APPROVER" | "FINAL_CONFIRMER"; onClose: () => void; onAssigned: () => void }) {
  ...
  const nextTitle = action === "PLAN_APPROVER" ? "諛?먭났媛??묒뾽?덇? - (怨꾪쉷?뺤씤) ?덇????쒕챸 ?붿껌" : "諛?먭났媛??묒뾽?덇? - (?댄뻾?뺤씤) 理쒖쥌 ?뺤씤 ?붿껌";
...
function DocumentContent({ doc, fd, approvalLines }: { doc: DocumentDetail; fd: Record<string, unknown>; approvalLines: ApprovalLine[] }) {
  ...
  const factorLabels: Record<string, string> = {
    factorNarrowAccess: "?묎렐?듬줈 ?묒냼", factorSlippery: "誘몃걚?ъ슫 吏諛?, factorSteepSlope: "湲됯꼍?щ㈃", factorWaterHazard: "?듭닔쨌?좎닔",
    factorRockfall: "?숈꽍쨌援대윭?⑥뼱吏?, factorNoRailing: "?덉쟾 ?쒓컙??, factorLadderNoGuard: "?щ떎由??덉쟾?좉툑?μ튂", factorSuffocation: "吏덉떇쨌?곗냼寃고븤쨌?좏빐媛??,
    factorElectricFire: "媛먯쟾쨌?꾧린?붿옱?붿씤", factorSparkFire: "遺덇퐙쨌遺덊떚???섑븳 ?붿옱", factorOther: `湲고?${fd.factorOtherDetail ? "(" + fd.factorOtherDetail + ")" : ""}`,
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {isForm1 && checkedFactors.length > 0 && <div className="bg-white rounded-2xl p-4 shadow-sm"><h3 className="text-sm font-bold text-gray-900 mb-3">諛쒖깮?섎뒗 ?꾪뿕?붿냼</h3><div className="flex flex-wrap gap-2">{checkedFactors.map((f, i) => (<span key={i} className="text-xs px-2.5 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-200">{f}</span>))}</div></div>}
      {isForm1 && Array.isArray(fd.riskRows) && (fd.riskRows as any[]).some(r => r.riskFactor || r.improvement) && <div className="bg-white rounded-2xl p-4 shadow-sm"><h3 className="text-sm font-bold text-gray-900 mb-3">?꾪뿕?붿냼 쨌 媛쒖꽑?梨?/h3><div className="space-y-2">{(fd.riskRows as any[]).filter(r => r.riskFactor || r.improvement).map((row, i) => (<div key={i} className="bg-gray-50 rounded-xl p-3 text-xs space-y-1">{row.riskFactor && <div><span className="text-gray-500">?꾪뿕?붿냼:</span> <span className="text-gray-800">{row.riskFactor}</span></div>}{row.improvement && <div><span className="text-gray-500">媛쒖꽑?梨?</span> <span className="text-gray-800">{row.improvement}</span></div>}{row.disasterType && <div><span className="text-gray-500">?ы빐?뺥깭:</span> <span className="text-gray-800">{row.disasterType}</span></div>}</div>))}</div></div>}
      {(isForm2 || isForm4) && Array.isArray(fd.safetyChecks) && <div className="bg-white rounded-2xl p-4 shadow-sm"><h3 className="text-sm font-bold text-gray-900 mb-3">?덉쟾議곗튂 ?댄뻾?ы빆</h3><div className="space-y-1.5">{(fd.safetyChecks as any[]).map((item, i) => (<div key={i} className="flex items-center gap-2 text-xs py-1 border-b border-gray-50"><div className={`w-4 h-4 rounded flex items-center justify-center shrink-0 ${item.applicable === "?대떦" ? "bg-blue-600" : "bg-gray-200"}`}>{item.applicable === "?대떦" && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}</div><span className={`flex-1 ${item.applicable === "?대떦" ? "text-gray-800 font-medium" : "text-gray-400"}`}>{item.label}</span>{item.applicable && <span className={`text-xs px-1.5 py-0.5 rounded ${item.applicable === "?대떦" ? "bg-blue-50 text-blue-600" : "bg-gray-100 text-gray-400"}`}>{item.applicable}</span>}{item.result && <span className="text-gray-500 text-xs">??{item.result}</span>}</div>))}</div></div>}
      {isForm4 && Array.isArray(fd.inspectionItems) && (fd.inspectionItems as any[]).some(i => i.equipment) && <div className="bg-white rounded-2xl p-4 shadow-sm"><h3 className="text-sm font-bold text-gray-900 mb-3">湲곌린 ?뺤씤 寃곌낵</h3><div className="overflow-x-auto"><table className="w-full text-xs"><thead><tr className="bg-gray-50"><th className="text-left px-2 py-1.5 text-gray-600 font-medium">?먭?湲곌린</th><th className="text-left px-2 py-1.5 text-gray-600 font-medium">李⑤떒?뺤씤??/th><th className="text-left px-2 py-1.5 text-gray-600 font-medium">?꾧린?대떦??/th><th className="text-left px-2 py-1.5 text-gray-600 font-medium">?꾩옣?뺣퉬</th></tr></thead><tbody>{(fd.inspectionItems as any[]).filter(i => i.equipment).map((item, i) => (<tr key={i} className="border-t border-gray-100"><td className="px-2 py-1.5 text-gray-800">{item.equipment}</td><td className="px-2 py-1.5 text-gray-800">{item.cutoffConfirmer}</td><td className="px-2 py-1.5 text-gray-800">{item.electrician}</td><td className="px-2 py-1.5 text-gray-800">{item.siteRepair}</td></tr>))}</tbody></table></div></div>}
      {isForm3 && Array.isArray(fd.participants) && <div className="bg-white rounded-2xl p-4 shadow-sm"><h3 className="text-sm font-bold text-gray-900 mb-3">?묒뾽 李몄뿬??/h3><div className="space-y-1.5">{(fd.participants as any[]).map((p, i) => (<div key={i} className="flex gap-3 text-sm"><span className="text-gray-400 w-28 shrink-0">{p.role}</span><span className="text-gray-900">{p.name} {p.phone ? `(${p.phone})` : ""}</span></div>))}</div></div>}
      {isForm3 && (fd.riskFactors || fd.improvementMeasures) && <div className="bg-white rounded-2xl p-4 shadow-sm"><h3 className="text-sm font-bold text-gray-900 mb-3">?꾪뿕?붿냼 諛?媛쒖꽑?梨?/h3><div className="space-y-2">{fd.riskFactors && <Field label="?꾪뿕?붿냼" value={fd.riskFactors as string} />}{fd.improvementMeasures && <Field label="媛쒖꽑?梨? value={fd.improvementMeasures as string} />}</div></div>}
      {(isForm2 || isForm4) && fd.specialMeasures && doc.currentApprovalOrder !== 1 && doc.status !== "SUBMITTED" && <div className="bg-white rounded-2xl p-4 shadow-sm"><h3 className="text-sm font-bold text-gray-900 mb-2">?밸퀎議곗튂 ?꾩슂?ы빆</h3><p className="text-sm text-gray-800">{fd.specialMeasures as string}</p></div>}
    ...
    <div className="border border-gray-200 rounded-xl p-3 space-y-3 bg-gray-50">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex gap-4">
          <NumSpinner field="hour" value={row.hour || ""} min={0} max={23} label="?? />
          <NumSpinner field="minute" value={row.minute || ""} min={0} max={59} label="遺? />
        </div>
      </div>
      {/* 媛???띾룄 媛쒕퀎 ?낅젰 */}
      <div>
        <p className="text-xs font-medium text-gray-600 mb-2">痢≪젙 ?띾룄</p>
        <div className="grid grid-cols-2 gap-2">
          <GasField field="o2"  label="?곗냼 O??      unit="%" placeholder="18~23.5" onCommit={v => onChange(idx, "o2",  v)} />
          <GasField field="co2" label="?댁궛?뷀깂??CO?? unit="%" placeholder="1.5誘몃쭔"  onCommit={v => onChange(idx, "co2", v)} />
          <GasField field="h2s" label="?⑺솕?섏냼 H?괪"   unit="ppm" placeholder="10誘몃쭔" onCommit={v => onChange(idx, "h2s", v)} />
          <GasField field="co"  label="?쇱궛?뷀깂??CO"  unit="ppm" placeholder="30誘몃쭔" onCommit={v => onChange(idx, "co",  v)} />
          <GasField field="ex"  label="??컻?섑븳 EX"    unit="%" placeholder="10誘몃쭔"   onCommit={v => onChange(idx, "ex",  v)} />
        </div>
      </div>
      {/* 痢≪젙??+ ?몄썝 */}
      <div className="grid grid-cols-3 gap-2">
        <div>
          <label className="text-xs text-gray-500 mb-1 block">痢≪젙??/label>
          <MeasurerField field="measurer" value={row.measurer || ""} />
        </div>
        <div>
          <label className="text-xs text-gray-500 mb-1 block">???곸궋(嶺?</label>
        ...
        <div>
          <label className="text-xs text-gray-500 mb-1 block">??怨몄궋(嶺?</label>
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-gray-500 bg-gray-50 rounded-lg px-3 py-2">
        ??⑤챷???瑜귣뭵: O??18~23.5%) CO??1.5%亦껋꼶梨뜹퐲? H???10ppm亦껋꼶梨뜹퐲? CO(30ppm亦껋꼶梨뜹퐲? EX(10%亦껋꼶梨뜹퐲?
      </p>
      {rows.map((row, idx) => (
    ...
    return (
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-blue-100">
        ...
        <div className="space-y-3">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1.5">?브퀗??洹쏆물?⑤슢沅?/label>
            <textarea
              key={`result-${dataKey}`}
              ref={reviewResultRef}
              defaultValue={reviewResult}
              placeholder="?브퀗??洹쏆물?⑤슢沅?????놁졑??怨삵룖?筌뤾쑴??
              rows={2}
              className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none text-gray-900" />
          </div>
        </div>
    ...
    <div className="pb-40">
      <div className="px-4 pt-4 pb-3 bg-white border-b border-gray-100">
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          {isMyTurn && <span className="text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-600 font-medium animate-pulse">??嶺뚢뼰維?</span>}
        </div>
        ...
        <div className="fixed bottom-16 left-0 right-0 bg-white border-t border-gray-200 px-4 pt-3 pb-4">
          ...
          {isConfinedSpace && confinedOrder === 3 ? (
            <button onClick={async () => {
              ...
              {processing ? "痍⑥냼 以?.." : "?뱤 痢≪젙寃곌낵 ?쒖텧 諛??댄뻾?뺤씤??吏??}
            </button>
          ) : (
            <div className="flex gap-3">
              <button onClick={() => {
      ...
      {showRejectConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.5)" }}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowRejectConfirm(false)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-600">???쳛??/button>
      ...
      {showApproveConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.5)" }}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowApproveConfirm(false)} className="flex-1 py-2.5 rounded-xl border border-gray-200 text-sm text-gray-600">???쳛??/button>
              <button onClick={() => handleAction("APPROVE")} className="flex-1 py-2.5 rounded-xl text-sm font-medium text-white" style={{ background: "#16a34a" }}>?筌먦끉逾?/button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white w-full rounded-t-3xl" style={{ paddingBottom: "env(safe-area-inset-bottom, 20px)", maxHeight: "80vh", overflowY: "auto" }}>
            ...
            <div className="px-6 pb-24 space-y-2">
              <div className="flex gap-2">
                ...
                <button onClick={() => setShowSign(false)} className="flex-1 py-3 rounded-xl border border-gray-200 text-sm text-gray-600 font-medium">???쳛??/button>
              </div>
