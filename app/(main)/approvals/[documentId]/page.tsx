...
{(isForm2 || isForm4) && Array.isArray(fd.safetyChecks) && (
      ...
      {(fd.reviewOpinion || fd.reviewResult) && <div className="bg-white rounded-2xl p-4 shadow-sm"><h3 className="text-sm font-bold text-gray-900 mb-3">검토의견</h3><div className="space-y-2">{fd.reviewOpinion && <div><p className="text-xs text-gray-500 mb-1">검토의견</p><p className="text-sm text-gray-800 bg-gray-50 rounded-xl px-3 py-2">{fd.reviewOpinion as string}</p></div>}{fd.reviewResult && <div><p className="text-xs text-gray-500 mb-1">조치결과</p><p className="text-sm text-gray-800 bg-gray-50 rounded-xl px-3 py-2">{fd.reviewResult as string}</p></div>}</div></div>}
...
export default function ApprovalDetailPage() {
  ...
  const [gasMeasureRowsInput, setGasMeasureRowsInput] = useState<any[]>(() => {
    const measurerName = (fd as any)?.measurerName || "";
    if (Array.isArray((fd as any)?.gasMeasureRows) && (fd as any).gasMeasureRows.length > 0) {
      return (fd as any).gasMeasureRows.map((r: any) => ({ ...r, measurer: r.measurer || measurerName }));
    }
    return [];
  });
            ...
            <div className="space-y-3">
              ...
              <GasMeasureInput
                rows={gasMeasureRowsInput.length > 0 ? gasMeasureRowsInput :
                  DEFAULT_GAS_ROWS.map((r) => ({ ...r, measurer: (fd.measurerName as string) || "" }))}
