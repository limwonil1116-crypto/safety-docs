    ...
    return (
      <div className="space-y-3">
        <p className="text-xs bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">
          <span className="font-medium text-blue-700">?ì •?˜ì¹˜:</span>
          <span className="text-blue-600"> O??18~23.5%) CO??1.5%ë¯¸ë§Œ) H?‚S(10ppmë¯¸ë§Œ) CO(30ppmë¯¸ë§Œ) EX(10%ë¯¸ë§Œ)</span>
        </p>
        {rowsRef.current.map((row, idx) => (
          <GasRowInput key={`${idx}-${row.o2}-${row.measurer}`} rowIndex={idx} initialRow={row} onRowChange={handleFieldChange}
            phase={["?‘ì—… ??, "?‘ì—… ì¤?1ì°?", "?‘ì—… ì¤?2ì°?"][idx]}
            onSave={documentId && fd ? async () => {
              const allRows = rowsRef.current;
              const phase = ["?‘ì—… ??, "?‘ì—… ì¤?1ì°?", "?‘ì—… ì¤?2ì°?"][idx];
              ...
              if (res.ok) { alert(`[${phase}] ?€???„ë£Œ!`); return true; }
              else alert("?€???¤íŒ¨.");
            } : undefined} />
        ))}
      </div>
