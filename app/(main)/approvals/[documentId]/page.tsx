...
function AttachmentViewer({ documentId, canAdd = false }: { documentId: string; canAdd?: boolean }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm">
      ...
      {photos.length > 0 && <div className="mb-4"><p className="text-xs text-gray-400 mb-2">📷 사진 {photos.length}장</p><div className="grid grid-cols-3 gap-2">{photos.map(photo => (<div key={photo.id} className="relative aspect-square rounded-xl overflow-hidden border border-gray-200 bg-gray-50 cursor-pointer active:opacity-80" onClick={() => setPreviewUrl(photo.fileUrl)}><img src={photo.fileUrl} alt={photo.fileName} className="w-full h-full object-cover" /></div>))}</div></div>}
      {docFiles.length > 0 && <div className="mb-3"><p className="text-xs text-gray-400 mb-2">📄 문서 {docFiles.length}개</p><div className="space-y-2">{docFiles.map(doc => { const isPdf = doc.mimeType==="application/pdf"; const isExcel = doc.mimeType?.includes("excel")||doc.mimeType?.includes("spreadsheet"); return (<a key={doc.id} href={doc.fileUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-200 hover:bg-blue-50 transition-colors"><div className={`w-9 h-9 rounded-lg flex items-center justify-center text-white text-xs font-bold shrink-0 ${isPdf?"bg-red-500":isExcel?"bg-green-600":"bg-blue-500"}`}>{isPdf?"PDF":isExcel?"XLS":"DOC"}</div><div className="flex-1 min-w-0"><p className="text-sm text-gray-800 font-medium truncate">{doc.fileName}</p><p className="text-xs text-gray-400">{formatSize(doc.fileSize)}</p></div></a>); })}</div></div>}
      {canAdd && <div className="flex gap-2 mt-2 pt-3 border-t border-gray-100"><button onClick={() => cameraRef.current?.click()} disabled={uploading} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-gray-300 text-sm text-gray-600 hover:bg-gray-50 disabled:opacity-50">카메라</button><button onClick={() => galleryRef.current?.click()} disabled={uploading} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl border border-gray-300 text-sm text-gray-600 hover:bg-gray-50 disabled:opacity-50">갤러리</button></div>}
    ...
    <div className="border border-gray-200 rounded-xl p-3 space-y-3 bg-gray-50">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <GasField field="o2"  label="?곗냼 O??      unit="%" placeholder="18~23.5" />
          <GasField field="co2" label="?댁궛?뷀깂??CO?? unit="%" placeholder="1.5誘몃쭔" />
          <GasField field="h2s" label="?⑺솕?섏냼 H?괪"   unit="ppm" placeholder="10誘몃쭔" />
          <GasField field="co"  label="?쇱궛?뷀깂??CO"  unit="ppm" placeholder="30誘몃쭔" />
          <GasField field="ex"  label="??컻?섑븳 EX"    unit="%" placeholder="10誘몃쭔" />
        </div>
      </div>
      {/* 痢≪젙??+ ?몄썝 */}
      <div className="grid grid-cols-3 gap-2">
        ...
        <div>
          <label className="text-xs text-gray-500 mb-1 block">?낆옣(紐?</label>
        ...
        <div>
          <label className="text-xs text-gray-500 mb-1 block">?댁옣(紐?</label>
...
function GasMeasureInput({ rows, onChange }: { rows: any[]; onChange: (rows: any[]) => void }) {
    ...
    if (isConfinedSpace) {
      ...
      return (
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-blue-100">
          ...
          {confinedOrder === 1 && (
            <p className="text-xs text-blue-600 bg-blue-50 rounded-lg px-3 py-2">媛먯떆?몄쑝濡쒖꽌 작업 怨꾪쉷???뺤씤?섍퀬 ?쒕챸?댁＜?몄슂.</p>
          )}
          {confinedOrder === 2 && (
            <div className="space-y-2">
        ...
        <div className="space-y-3">
          <div>
            ...
            <label className="block text-xs font-medium text-gray-600 mb-1.5">
              寃?좎쓽寃?{doc.currentApprovalOrder === 1 && <span className="text-red-500 text-xs">(반려 ???꾩닔)</span>}
            </label>
            <textarea
              key={`opinion-${dataKey}`}
              ref={reviewOpinionRef}
              defaultValue={reviewOpinion}
              placeholder="寃???섍껄???낅젰?댁＜?몄슂 (반려 ???꾩닔)"
              rows={3}
  ...
  const CancelButton = () => canCancel ? (
    <button onClick={handleCancelApproval} disabled={cancelling}
      ...
      {cancelling ? "痍⑥냼 이후.." : "寃곗옱 痍⑥냼 (?묒꽦以묒쑝濡?"}
    </button>
  ) : null;
  ...
  return (
    <div className="pb-40">
      ...
      <div className="p-4 space-y-4">
        ...
        {/* ??3踰? 결재현황 탭- 寃곗옱?먮쫫 + ?밸퀎議곗튂(2/3?④퀎) + ?ъ쭊 */}
        {activeTab === "寃곗옱?꾪솴" && (
          <>
        ...
        <div className="fixed bottom-16 left-0 right-0 bg-white border-t border-gray-200 px-4 pt-3 pb-4">
          ...
          {isConfinedSpace && confinedOrder === 3 ? (
            <button onClick={async () => {
              ...
              {processing ? "?쒖텧 이후.." : "?뱤 痢≪젙寃곌낵 ?쒖텧 諛??댄뻾?뺤씤??吏??}
            </button>
          ) : (
            <div className="flex gap-3">
              <button onClick={() => {
              ...
              <button onClick={() => {
                ...
                {isConfinedSpace
                  ? confinedOrder === 1 ? "감시인"?쒕챸" : confinedOrder === 2 ? "(怨꾪쉷?뺤씤) ?쒕챸" : "(?댄뻾?뺤씤) 理쒖쥌 ?쒕챸"
      ...
      {showRejectConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.5)" }}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            <h3 className="text-base font-bold text-gray-900 mb-2">반려?섏떆寃좎뒿?덇퉴?</h3>
            <p className="text-sm text-gray-500 mb-4">반려 泥섎━ ???좎껌?몄뿉寃??뚮┝???꾩넚?⑸땲??</p>
            {!(reviewOpinionRef.current?.value ?? reviewOpinion).trim() && <p className="text-xs text-red-500 mb-3">반려 ?ъ쑀(寃?좎쓽寃?瑜?癒쇱? ?낅젰?댁＜?몄슂.</p>}
            <div className="flex gap-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.5)" }}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
            <h3 className="text-base font-bold text-gray-900 mb-2">
              {isConfinedSpace
                ? confinedOrder === 1 ? "감시인"?쒕챸 ??(怨꾪쉷?뺤씤)?덇??먮? 吏?뺥빀?덈떎"
                  : confinedOrder === 2 ? "(계획확인) 허가자?쒕챸???꾨즺?⑸땲??
                  : "(?댄뻾?뺤씤) 理쒖쥌 ?쒕챸???꾨즺?⑸땲??
        ...
        <div ref={signModalRef} className="fixed inset-0 bg-black/50 z-50 flex items-end" style={{ touchAction: "none" }} onTouchMove={e => e.preventDefault()}>
          <div className="bg-white w-full rounded-t-3xl" style={{ paddingBottom: "env(safe-area-inset-bottom, 20px)", maxHeight: "80vh", overflowY: "auto" }}>
            <div className="px-6 pt-6 pb-2">
              <h2 className="text-base font-bold text-gray-900 mb-1">{pendingAction === "APPROVE" ? "승인 서명" : "반려 ?쒕챸"}</h2>
            ...
            <div className="px-6 pb-24 space-y-2">
              ...
              <button onClick={handleSubmitWithSign} disabled={processing}
                ...
                {processing ? "泥섎━ 이후.." : pendingAction === "APPROVE" ? "???뱀씤 ?꾨즺" : "반려 ?꾨즺"}
              </button>
            </div>
          </div>
        </div>
