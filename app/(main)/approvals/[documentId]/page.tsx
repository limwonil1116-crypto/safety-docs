  ...
  return (
    <div>
      <button onClick={handleGenerate} disabled={loading}
        ...
        {loading ? (
          <><svg className="animate-spin" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>AI 초안 생성 중...</>
        ) : (
          <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg>✨ {label}</>
        )}
      </button>
...
function SpecialMeasuresInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  return (
    <textarea ref={ref} defaultValue={value}
      ...
      onCompositionEnd={e => { composing.current = false; onChange((e.target as HTMLTextAreaElement).value); }}
    ...
    <div className="border border-gray-200 rounded-xl p-3 space-y-3 bg-gray-50">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="flex flex-col"><label className="text-[10px] text-gray-500 mb-0.5">산소 O₂</label><div className="flex items-center gap-0.5"><input type="text" defaultValue={row.o2||""} onChange={e=>onChange(idx,"o2",e.target.value)} onBlur={e=>onChange(idx,"o2",e.target.value)} placeholder="18~23.5" className="w-full px-2 py-2 text-sm text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white" /><span className="text-xs text-gray-500 shrink-0 ml-0.5">%</span></div></div>
          <div className="flex flex-col"><label className="text-[10px] text-gray-500 mb-0.5">이산화탄소 CO₂</label><div className="flex items-center gap-0.5"><input type="text" defaultValue={row.co2||""} onChange={e=>onChange(idx,"co2",e.target.value)} onBlur={e=>onChange(idx,"co2",e.target.value)} placeholder="1.5미만" className="w-full px-2 py-2 text-sm text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white" /><span className="text-xs text-gray-500 shrink-0 ml-0.5">%</span></div></div>
          <div className="flex flex-col"><label className="text-[10px] text-gray-500 mb-0.5">황화수소 H₂S</label><div className="flex items-center gap-0.5"><input type="text" defaultValue={row.h2s||""} onChange={e=>onChange(idx,"h2s",e.target.value)} onBlur={e=>onChange(idx,"h2s",e.target.value)} placeholder="10미만" className="w-full px-2 py-2 text-sm text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white" /><span className="text-xs text-gray-500 shrink-0 ml-0.5">ppm</span></div></div>
          <div className="flex flex-col"><label className="text-[10px] text-gray-500 mb-0.5">일산화탄소 CO</label><div className="flex items-center gap-0.5"><input type="text" defaultValue={row.co||""} onChange={e=>onChange(idx,"co",e.target.value)} onBlur={e=>onChange(idx,"co",e.target.value)} placeholder="30미만" className="w-full px-2 py-2 text-sm text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white" /><span className="text-xs text-gray-500 shrink-0 ml-0.5">ppm</span></div></div>
          <div className="flex flex-col"><label className="text-[10px] text-gray-500 mb-0.5">폭발하한 EX</label><div className="flex items-center gap-0.5"><input type="text" defaultValue={row.ex||""} onChange={e=>onChange(idx,"ex",e.target.value)} onBlur={e=>onChange(idx,"ex",e.target.value)} placeholder="10미만" className="w-full px-2 py-2 text-sm text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-400 bg-white" /><span className="text-xs text-gray-500 shrink-0 ml-0.5">%</span></div></div>
        </div>
      </div>
      {/* 측정자 + 인원 */}
      <div className="grid grid-cols-3 gap-2">
...
export default function ApprovalDetailPage() {
  ...
  useEffect(() => {
    if (dataKey > 0 && reviewOpinionRef.current) reviewOpinionRef.current.value = reviewOpinion;
    if (dataKey > 0 && reviewResultRef.current) reviewResultRef.current.value = reviewResult;
  }, [dataKey]);
  ...
  const handleSubmitWithSign = async () => {
    ...
    try {
      ...
      else if (data.action === "NEED_MEASUREMENT") { alert("(계획확인) 서명이 완료됩니다. 측정담당자에게 측정결과 입력을 요청합니다."); router.push("/approvals"); }
