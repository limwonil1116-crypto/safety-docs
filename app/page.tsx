...
export default function Home() {
  ...
  if (status === "loading") {
    return <div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="text-gray-400 text-sm">로딩 중...</div></div>;
  }

  if (!session) return null;

  const handleSelect = (category: "CONTRACTOR" | "SELF") => {
    setSelected(category);
    if (category === "CONTRACTOR") router.push("/tasks?category=CONTRACTOR");
    else router.push("/tasks?category=SELF");
  };
