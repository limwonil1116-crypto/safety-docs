...
export default function Header() {
  ...
  useEffect(() => {
    ...
    // Android (Chrome / Samsung Internet 포함)
    ...
    // Android인데 이미 캐시된 prompt가 있을 수 있으니 idle 상태 유지 (버튼은 보임)
  ...
  const handleInstall = async () => {
    ...
    // prompt 없을 경우에만 안내 모달 표시
    setShowModal(true);
  };

  if (installState === "installed") {
    return <HeaderShell showInstallBtn={false} onInstall={handleInstall} />;
  }
...
function InstallModal({
      ...
      <ModalWrapper onClose={onClose}>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 mb-4">
          ...
          <p className="text-xs text-yellow-700">
            카카오톡 내에서는 앱 설치가 불가합니다. 아래 방법으로 외부 브라우저에서 열어주세요.
          </p>
        </div>
    ...
    return (
      <ModalWrapper onClose={onClose}>
        ...
        <p className="text-sm text-gray-600 mb-4">
          인앱 브라우저에서는 설치가 제한됩니다. Chrome 또는 Safari로 접속 후 설치해 주세요.
        </p>
  ...
  // Android - beforeinstallprompt가 아직 안 온 경우 (Chrome 설치 조건 미충족 등)
  return (
    <ModalWrapper onClose={onClose}>
      <h2 className="text-base font-bold text-gray-900 mb-3">앱 설치 안내</h2>
      <p className="text-sm text-gray-600 mb-4">
        Chrome 브라우저에서 접속하신 경우, 주소창 우측의 설치 아이콘을 탭하거나
        메뉴(⋮)에서 <strong>앱 설치</strong>를 선택하세요.
      </p>
      <div className="bg-blue-50 rounded-xl p-3 mb-4 text-xs text-blue-700">
        💡 Samsung Internet 사용 중이시라면 하단 메뉴(≡) → 페이지 추가 → 홈 화면에 추가
      </div>
      <button
        onClick={onClose}
        className="w-full py-3 rounded-xl text-white font-medium text-sm"
