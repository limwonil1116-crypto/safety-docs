...
export const tbmReports = pgTable("tbm_reports", {
  id: uuid("id").primaryKey().defaultRandom(),
  taskId: uuid("task_id").references(() => tasks.id, { onDelete: "cascade" }),
  submittedBy: uuid("submitted_by").references(() => users.id),
  reportDate: varchar("report_date", { length: 20 }).notNull(),
  eduStartTime: varchar("edu_start_time", { length: 10 }),
  eduEndTime: varchar("edu_end_time", { length: 10 }),
  ...
  workToday: text("work_today"),
  workAddress: varchar("work_address", { length: 500 }),
  workLatitude: varchar("work_latitude", { length: 30 }),
  workLongitude: varchar("work_longitude", { length: 30 }),
  workerCount: integer("worker_count").default(0),
  newWorkerCount: integer("new_worker_count").default(0),
  equipment: text("equipment"),
  riskType: varchar("risk_type", { length: 100 }),
  cctvUsed: boolean("cctv_used").default(false),
  riskFactor1: text("risk_factor1"),
  riskMeasure1: text("risk_measure1"),
  riskFactor2: text("risk_factor2"),
  riskMeasure2: text("risk_measure2"),
  riskFactor3: text("risk_factor3"),
  riskMeasure3: text("risk_measure3"),
  mainRiskFactor: text("main_risk_factor"),
  mainRiskMeasure: text("main_risk_measure"),
  riskElement1: text("risk_element1"),
  riskElement2: text("risk_element2"),
  riskElement3: text("risk_element3"),
